...
def muenz_wechsel(betrag, min_muenze):
    muenzen = [1, 2, 5, 10, 20, 50, 100, 200]  # Werte der Münzen in Cent
    muenzen = [m for m in muenzen if m >= min_muenze]  # Filtere Münzen nach min_muenze

    speicher = {}  # Wörterbuch zur Speicherung bereits berechneter Ergebnisse

    def hilfsfunktion(betrag, min_muenze, max_muenze):
        if (betrag, min_muenze, max_muenze) in speicher:
            return speicher[(betrag, min_muenze, max_muenze)]

        if betrag == 0:
            return [[]]  # Basisfall: Betrag erreicht

        ergebnisse = []
        for muenze in muenzen:
            if muenze > max_muenze or muenze < min_muenze:
                continue
            if muenze <= betrag:
                for kombination in hilfsfunktion(betrag - muenze, muenze, max_muenze):
                    ergebnisse.append([muenze] + kombination)

        speicher[(betrag, min_muenze, max_muenze)] = ergebnisse
        return ergebnisse

    roher_output = hilfsfunktion(betrag, min_muenze, max(muenzen))

    # Formatierung der Ausgabe
    formatierter_output = []
    for kombination in roher_output:
        formatierte_kombination = []
        for muenze in kombination:
            if muenze >= 100:
                formatierte_kombination.append(f"{muenze // 100} Euro")
            else:
                formatierte_kombination.append(f"{muenze} Cent")
        formatierter_output.append(formatierte_kombination)

    return formatierter_output
